...
class ClaudeAgent(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        # Only ever respond to plomdawg; ignore everyone else (incl. the bot itself).
        ...
        try:
            ...
            self.log(f"Failed to create/resolve thread: {e}")
            return

        await self._run_guarded(thread, prompt)

    async def _run_guarded(self, thread: discord.Thread, prompt: str):
        """Serialize runs per-thread and bound total concurrency."""
        lock = self.locks.setdefault(thread.id, asyncio.Lock())
        async with self.global_sem, lock:
            try:
                await self._run(thread, prompt)
